...
void analyseExtDef(TreeNode *extDef, SymbolTable *st)
{
	...
	if (strcmp(node->name, "ExtDecList") == 0) {
		...
		if (strcmp(funNode->name, "RP") == 0) {
			...
			for (int i = 0; i < paramTypes->u.function.paramNum; i++) {
				t->u.function.params[i] = paramTypes->u.function.params[i];
			}
		}
...
void analyseDef(TreeNode *def, FieldList *structField, SymbolTable *st)
{
	...
	while (decNode != NULL) {
		...
		Type *varDecType = analyseVarDec(varDec, specifierType, structField, st);
		if (varDecType != NULL && varDec->nextSibling != NULL) {  // Dec : VarDec ASSIGNOP Exp
...
Type *analyseVarDec(TreeNode *varDec, Type *baseType, FieldList *structField, SymbolTable *st)
{
	/* VarDec : ID
	 * 		  | VarDec LB INT RB
	 * 		  ;
...
Type *analyseVarDecType(TreeNode *varDec, Type *baseType, SymbolTable *st)
{
	...
	if (node->type == TOKEN_ID) {
		...
		t->kind = ARRAY;
		t->u.array.elem = analyseVarDecType(node, baseType, st);
		node = node->nextSibling;  // LB
		node = node->nextSibling;  // INT
		t->u.array.size = node->ival;
		return t;
	}
}
		...
		if (strcmp(node->name, "Exp") != 0) {
			if (strcmp(node->name, "LP") == 0) {  // LP Exp RP
				...
				if (strcmp(node1->name, "RP") == 0) {  // ID LP RP
					...
					TreeNode *args = node1;
					Type *argTypes = getArgTypes(args, st);
					if (argTypes == NULL) return NULL;
					if (argTypes->u.function.paramNum != field->type->u.function.paramNum) {  // ERROR 9: 函数调用参数个数不匹配
						char msg[SERROR_MSG_LEN];
						sprintf(msg, "Function \"%s\" is not applicable with %d arguments.", node->sval,
								argTypes->u.function.paramNum);
						serror(node, 9, msg);
						free(t);
						free(argTypes);
						return NULL;
					}
					for (int i = 0; i < argTypes->u.function.paramNum; i++) {
						if (!isSameType(argTypes->u.function.params[i],
								field->type->u.function.params[i])) {  // ERROR 9: 函数调用参数类型不匹配
							char msg[SERROR_MSG_LEN];
...
Type *getArgTypes(TreeNode *args, SymbolTable *st)
{
	/* Args : Exp COMMA Args
	 * 		| Exp
	 * 		;
	...
	while (exp != NULL) {
		...
		t->u.function.params[argCnt++] = expType;
...
Type *getVarTypes(TreeNode *varList, SymbolTable *st)
{
	...
	while (paramDec != NULL) {
		...
		Type *varDecType = analyseVarDec(varDec, specifierType, NULL, st);
		t->u.function.params[paramCnt++] = varDecType;
